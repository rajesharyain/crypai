...
class PostCreatorAgent:
    def __init__(self, model_name: str = "gpt-3.5-turbo", temperature: float = 0.7, model_type: str = "openai"):
        """Initialize the Post Creator Agent with OpenAI or DeepSeek AI model"""
        ...
        if self.llm:
            logger.info(f"✅ PostCreatorAgent initialized with {model_type} {model_name}")
        else:
            logger.warning(f"⚠️ PostCreatorAgent initialized without {model_type} - will use fallback post creation")
    ...
    def _initialize_llm(self):
        ...
        try:
            if self.model_type == "openai" and self.openai_api_key:
                ...
                logger.warning(f"⚠️ No {self.model_type} API key found - will use fallback post creation")
                self.llm = None
